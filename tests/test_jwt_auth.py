...
def test_login():
    ...
    if response.status_code == 200:
        ...
        print(
            f"   User: {token_data['user']['username']} ({token_data['user']['role']})"
        )
...
def test_secured_endpoint_without_token():
    ...
    prompt_data = {"prompt": "Hello, how are you?", "max_tokens": 50}
...
def test_secured_endpoint_with_token(token):
    ...
    response = requests.post(
        f"{BASE_URL}/secured-generate", json=prompt_data, headers=headers
    )
...
def compare_endpoints(token):
    ...
    prompt_data = {"prompt": "What is 2+2?", "max_tokens": 30}
