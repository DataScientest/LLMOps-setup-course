...
def get_default_model():
    ...
    logger.debug(f" Getting default model from {LITELLM_URL}")
    try:
        ...
        # Priority order: Groq Qwen first, then optional providers
        priority_models = [
            "groq-qwen-primary",
            "gpt-4o-secondary",
            "gemini-third",
    ...
    logger.debug(" Using fallback: groq-qwen-primary")
    return "groq-qwen-primary"
            ...
            if body:
                ...
                for pattern, pattern_type in sql_injection_patterns:
                    if re.search(pattern, body_str, re.IGNORECASE):
                        raise ValueError(
                            f"Suspicious request body detected: {pattern_type}"
                        )
    ...
    return {
        ...
        "health_status": {
            "status": (
                "healthy"
                if block_rate < 20
                else "warning" if block_rate < 50 else "critical"
            ),
            "active_protections": 4,
            "last_incident": (
                recent_incidents[-1]["timestamp"] if recent_incidents else None
            ),
        },
        "timestamp": current_time.isoformat(),
...
async def generate_text(prompt_request: SecurePromptRequest):
    ...
    logger.debug("Starting generation with model=%s", prompt_request.model)
    ...
    current_span = mlflow.get_current_active_span()
    logger.debug("MLflow span available=%s", current_span is not None)
    ...
    try:
        ...
        with mlflow.start_span("llm_call", span_type=SpanType.LLM) as span:
            logger.debug("Sending request to model=%s", prompt_request.model)
            ...
            logger.debug(f" LiteLLM response status: {litellm_response.status_code}")
            logger.debug(f" LiteLLM response headers: {dict(litellm_response.headers)}")

            litellm_response.raise_for_status()
            response_data = litellm_response.json()
            logger.debug("Parsed a successful LiteLLM response")
            ...
            span.add_event(
                SpanEvent(
                    "Received response from LiteLLM proxy",
                    attributes={
        ...
        with mlflow.start_span("output_processing") as span:
            ...
            try:
                logger.debug(f" Calculating cost for model: {actual_model}")
                cost = (
                    completion_cost(
                        model=actual_model,
                        prompt=prompt_request.prompt,
                ...
                logger.debug(f" Cost calculated successfully: {cost}")
            except Exception as e:
            ...
            current_span.set_outputs(
                {
                    ...
                    "security_status": (
                        "protected"
                        if prompt_request.enable_guardrails
                        else "unprotected"
                    ),
        ...
        return SecurePromptResponse(
            ...
            cost=cost,
            security_status=(
                "protected" if prompt_request.enable_guardrails else "unprotected"
            ),
            guardrails_triggered=guardrails_triggered,
        )
...
async def secured_generate_text(
    ...
    logger.debug(
        "Secured generation user=%s role=%s model=%s",
    ...
    with mlflow.start_span("input_processing") as span:
        span.add_event(
            SpanEvent(
                "Processing authenticated input request",
                attributes={
                    "model": prompt_request.model,
                    "prompt_preview": prompt_request.prompt[:100],
                    "authenticated_user": current_user["username"],
    ...
    try:
        ...
        with mlflow.start_span("llm_call", span_type=SpanType.LLM) as span:
            logger.debug(
                "Sending authenticated request to model=%s", prompt_request.model
            )
            ...
            span.add_event(
                SpanEvent(
                    "Sending authenticated request to LiteLLM proxy",
                    attributes={
                        "model": prompt_request.model,
                        "has_system_prompt": bool(prompt_request.system_prompt),
                        "has_response_format": bool(prompt_request.response_format),
            ...
            logger.debug("LiteLLM response status=%s", litellm_response.status_code)
